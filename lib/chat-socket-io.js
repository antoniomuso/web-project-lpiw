module.exports = (io, session, db) => {
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('message', async (message) => {
            ...
            socket.to(room).emit('message', { username: socket.handshake.session.username, message: message})
        ...
        socket.on('createRoom', async (nome, desc) => {
            ...
            socket.of('/').emit('createRoom', {ist: tab.rows[0].ist, name: nome, descr: desc})
