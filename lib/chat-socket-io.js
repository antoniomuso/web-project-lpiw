module.exports = (io, session, db) => {
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('join', async (room, callback) => {
            ...
            // Entro nella nuova
            socket.join(room)
            callback(true, socket.handshake.session.idUtente)
        })
        ...
        socket.on('message', async (message) => {
            ...
            socket.to(room).emit('message', { username: socket.handshake.session.username, message: messEscp})
        ...
        socket.on('createRoom', async (object, cb) => {
            ...
            cb(tab.rows[0].ist, socket.handshake.session.username)
